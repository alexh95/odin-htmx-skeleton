...
- HTMX is **embedded into the binary** (`#load` in `controllers.odin`) and served from memory;
...
| `src/repository/` | repository | Owns the store + id counter. CRUD primitives. Imports `models`. |
